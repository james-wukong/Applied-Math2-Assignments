...
def create_tensor_of_val(dimensions: tuple, val: float) -> torch.Tensor:
    """
    ...
    Returns:
        Tensor object
    """
    res = torch.ones(dimensions)

    return res * val

def calculate_elementwise_product(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    """
    ...
    Args:
        ...
        B: torch.Tensor
    Returns:
        res, torch.Tensor element-wise product
    """
    res = A * B
...
def calculate_matrix_product(X: torch.Tensor, W: torch.Tensor) -> torch.Tensor:
    """
    ...
    Args:
        ...
        W: torch.Tensor
    Returns:
        res, torch.Tensor product of two tensors
    """
    res = torch.matmul(X, torch.t(W))

    return res

def calculate_matrix_prod_with_bias(X: torch.Tensor, 
                                    W: torch.Tensor, 
...
def calculate_activation(sum_total: torch.Tensor) -> torch.Tensor:
    """
    Calculate a step function as an activation of the neuron.
    Hint: use PyTorch `heaviside` function.
    """
    res = sum_total.apply_(lambda x: 1.0 if x > 0 else 0.0)  # TODO: implement this function 
    return res
...
def calculate_output(X: torch.Tensor, 
    ...
    output = calculate_activation(res)
    return output
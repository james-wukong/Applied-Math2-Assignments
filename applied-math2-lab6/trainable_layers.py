...
def define_dense_model_with_hidden_layers(input_length,
    ...
    # x = layers.Dense(hidden_layers_sizes[0], activation=activation_func_array[0])(inputs)
    # x = layers.Dense(hidden_layers_sizes[1], activation=activation_func_array[1])(x)
    model.add(layers.Dense(hidden_layers_sizes[0], activation=activation_func_array[0], name='first-hidden-layer'))
    model.add(layers.Dense(hidden_layers_sizes[1], activation=activation_func_array[1], name='second-hidden-layer'))
    # Create the output layer
    # outputs = layers.Dense(output_length, activation=output_function)(x)
